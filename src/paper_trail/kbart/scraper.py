...
def download_single_kbart_file(args: tuple[str, int, int]) -> Path | None:
    """Worker task: Checks freshness and downloads a single KBART file."""
...
def fetch_all_kbart_files(max_workers: int = 8) -> list[Path]:
    """Discovers and downloads all KBART lists concurrently with progress tracking."""
    ...
    if not target_urls:
        print("No KBART URLs found.")
        return []

    print(f"Targeting {len(target_urls)} KBART file(s) with {max_workers} workers...")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        if HAS_TQDM:
            pbar = tqdm(
                total=len(tasks), desc="Scraping KBART Lists", unit="file", leave=True
            )
    ...
    print(f"Finished downloading {len(downloaded_files)} KBART file(s).")
    return downloaded_files


def cli_main():
    parser = argparse.ArgumentParser(
        description="Scrape and download KBART files concurrently."
    )
    parser.add_argument(
        "--workers",
        "-w",
